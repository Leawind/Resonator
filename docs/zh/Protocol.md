...
| `0` | `1`   | `'S'`        | `type`       | 消息类型                                                                                           |
| `1` | `3`   | `u24_be`     | `request_id` | 请求标识                                                                                           |
| `4` | `4+?` | `sized_utf8` | `plugin_id`  | 插件标识。前 4 字节是`u32`，表示 utf8 字符串的字节数，接下来这么多字节是 utf8 字符串，表示插件标识 |
...
第一个字节表示消息类型，接下来 3 个字节的含义取决于具体消息类型。
...
| --- | --- | -------- | -------------- | -------- |
| `0` | `1` | `'e'`    | `type`         | 消息类型 |
| `1` | `3` | `u24_be` | `unused`       | 无用     |
| `4` | `4` | `u32_be` | `subscribe_id` | 订阅标识 |
